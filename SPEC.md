...
> 文档职责：`INTENT.md` 说明为什么做和如何取舍；本文件只定义系统必须满足的可验收行为；
> `IMPL.md` 记录当前模块、算法、默认值与已知限制。实现改变不应直接改变 Intent，默认值改变也不应被误写成需求。
...
9. **字幕优先**：可用的平台人工字幕必须优先于自动字幕和 ASR；ASR 是回退而不是必经阶段。
10. **写作与检索分离**：固定窗口可用于检索，但不得直接充当章节边界。
11. **证据后重组**：写作必须先生成高召回内容映射和全局大纲，再按章节读取原始证据撰写。
12. **自适应视觉预算**：不得用视频级固定小常数限制语义视觉需求；预算必须按章节或请求分配。
13. **图文公式优先**：现阶段公式验收目标是召回清晰完整的原始画面并与正文配对，不要求公式转录。
...
- Authoring 模式必须先从 Content Map 生成带时间范围的视觉需求，再在各需求范围内选择候选帧。
- 候选预算按视觉需求分配，不得设置视频级固定帧数上限。
- 未配置或未启用 VLM 时，不得生成或展示图文时间轴，也不得把文本推测当作图片解释。
- 配置 VLM 时，应由 VLM 判断画面内容及图文相关性。
...
- 术语匹配不区分大小写。
- `timeline.json` 是检索与问答的证据视图；不得作为 Authoring 的唯一输入或章节边界。
